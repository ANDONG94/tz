 ...
 */
@Controller
@RequestMapping("TrainingSituationTobeComplete")
public class TrainingSituationTobeCompleteController {
    ...
    @RequestMapping("list")
    public String list() {
        return "tobecomplete/training_situation_tobe_complete_list";
    }
     ...
     * @return
     */
    @RequestMapping("queryTrainingSituationAccountByAab301")
    @ResponseBody
    public String queryTrainingSituationAccountByAab301(HttpServletRequest request, TsnPlf tsnPlf, HttpServletResponse response) {
        try {
            int pageSize = 50;
            int startRecord = 0;
            ...
            tsnPlf.setIstobecomplete("1");//待完善设置改为为1
            List<TsnPlf> list = trainingSituationService.queryTrainSituationAccountByAab301(tsnPlf, startRecord + "", pageSize + "");
            String countlist = trainingSituationService.queryAllTrainSituationByAab301(tsnPlf);
            DataTableResultVO<TsnPlf> result = new DataTableResultVO<>();
    ...
    @ResponseBody
    public String deleteTrainingSituationByTsn010(HttpServletRequest request, String tsn010, ModelMap modelMap) {
        int s = 0;
        if (!"".equals(tsn010) && tsn010 != null) {
            FpUser fpUser = (FpUser) request.getSession().getAttribute("fpUser");
            trainingSituationService.deleteTrainingSituationByTsn010(tsn010, fpUser.getRealname());
            s = 1;
        }
        if (s > 0) {
            return "yes";
